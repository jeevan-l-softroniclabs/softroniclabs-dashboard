...
FLASK_DEBUG=1  superset run -p 8088 --with-threads --reload --debugger
```
...
`FLASK_DEBUG=1 superset run -p 8088 --reload --debugger`
...
```bash
FLASK_DEBUG=1 gunicorn "superset.app:create_app()" -k "geventwebsocket.gunicorn.workers.GeventWebSocketWorker" -b 127.0.0.1:8088 --reload
```
